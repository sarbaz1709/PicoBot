...
- Note: As seen in the video, alternating modes doesn't work while servo is panning (as the code is in loop taking ultrasonic readings). Once servo stops panning the yellow LED lights up to indicate line following mode. This can be remedied with better code implementation. 
