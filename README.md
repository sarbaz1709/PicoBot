...
- Note: As seen in the video, alternating modes doesn't work while servo is panning (as the code is in loop taking ultrasonic readings). Once the servo stops panning the yellow LED lights up upon button press to indicate line following mode. This can be remedied with better code implementation. 
